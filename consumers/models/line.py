...
class Line:
    ...
    def process_message(self, message):
        ...
        if message.topic() == "chicago_transit.stations.table": # Set the conditional correctly to the stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif message.topic() == "chicago_transit.stations.arrival": # Set the conditional to the arrival topic
            self._handle_arrival(message)
        elif message.topic() == "chicago_transit.turnstile": # Set the conditional to the KSQL Turnstile Summary Topic
